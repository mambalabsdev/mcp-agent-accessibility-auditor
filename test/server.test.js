...
const TOOL_NAME = "audit_agent_accessibility";
const ACTOR_ID = "anxbRv0lKrpQ1pnua";
...
test("package identity matches the locked naming convention", () => {
  ...
  assert.deepEqual(key, ["mamba-agent-accessibility-auditor"]);
  assert.deepEqual(mcp.mcpServers[key[0]].args, ["-y", pkg.name]);
  assert.equal(pkg.name, "@mambalabsdev/mcp-agent-accessibility-auditor");
  assert.equal(pkg.mcpName, "com.mambabuilt/mcp-agent-accessibility-auditor");
});
